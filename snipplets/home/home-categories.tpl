{% if settings.main_categories and settings.slider_categories and settings.slider_categories is not empty %}
    ...
    <section class="section-home section-categories-home{% if settings.main_categories_colors or has_categories_background_image or has_categories_mobile_background_image %} section-home-color{% endif %} position-relative" data-store="home-categories-featured" data-transition="fade-in-up">
                            ...
                            {% endif %}
                                    <div class="home-category text-center">
                                        <div class="home-category-image">
                                            ...
                                            {# IMAGEM PRINCIPAL (sem alterações) #}
                                            <img src="{{ 'images/empty-placeholder.png' | static_url }}" data-src="{{ slide.image | static_url | settings_image_url('medium') }}" class="swiper-lazy category-image-main" alt="{{ 'Categoría' | translate }} {{ loop.index }}">
                                            
                                            {# IMAGEM DE HOVER (AQUI ESTÁ A CORREÇÃO) #}
                                            {# Usamos a função attribute() para acessar o índice de forma segura #}
                                            {% set hover_slide = attribute(settings.slider_categories_hovered, loop.index0) %}
                                            {% if hover_slide and hover_slide.image %}
                                                <img src="{{ 'images/empty-placeholder.png' | static_url }}" data-src="{{ hover_slide.image | static_url | settings_image_url('medium') }}" class="swiper-lazy category-image-hover" alt="{{ 'Categoría' | translate }} {{ loop.index }} Hover">
                                            {% endif %}
        ...
        {% if has_categories_background_image or has_categories_mobile_background_image %}
            {# ... (o resto do código de background images continua o mesmo) ... #}
